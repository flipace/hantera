...
var _ = Describe("commands.develop.Update", func() {
	...
	It("throws when no hantera.yml can be found", func() {
		testFunc := func() {
			develop.RunUpdate(
				filepath.Join(targetPath, "nono.yml"),
				targetPath,
				"master",
	...
	It("clones \"new dependencies\" into the dependency-name folder beneath the target folder", func() {
		develop.RunSetup(
			filepath.Join(targetPath, "hantera.yml"),
			targetPath,
			"master",
		...
		// add a new dependency to the config
		d1 := []byte("name: testconfig\n" +
			"dependencies:\n" +
			"  themeit:\n" +
			"    repository: https://github.com/flipace/react-themeit\n" +
			"  whoport:\n" +
			"    repository: https://github.com/flipace/whoport\n")
		...
		Expect(filepath.Join(targetPath, "whoport")).To(BeADirectory())
		Expect(filepath.Join(targetPath, "whoport", "package.json")).To(BeARegularFile())
	})
})
